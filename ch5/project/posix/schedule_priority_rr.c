...
// CSC 139
// Spring 2026
// Scheduler Priority with Round Robin
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
Task *pickNextTask(void) {
    ...
    int max_prio = head->task->priority;
    for (struct node *p = head->next; p; p = p->next)
        if (p->task->priority > max_prio) max_prio = p->task->priority;

    for (struct node *p = head; p; p = p->next)
        if (p->task->priority == max_prio) return p->task;
